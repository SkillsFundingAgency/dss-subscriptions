    ...
    public class PatchSubscriptionsHttpTrigger
    {
        ...
        public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = "Customers/{customerId}/Subscriptions/{subscriptionId}")] HttpRequest req, string customerId, string subscriptionId)
        {
            ...
            if (!Guid.TryParse(subscriptionId, out var subscriptionsGuid))
            {
                ...
                _logger.LogInformation("{CorrelationId} Response Status Code: {StatusCode}. Unable to parse 'subscriptionId' to a Guid: {subscriptionId}", correlationId, response.StatusCode, subscriptionId);
                return response;
            }
            ...
            _logger.LogTrace("{CorrelationId} Attempting to get Subscriptions for customer {customerGuid} and subscription with {subscriptionsGuid}", correlationId, customerGuid, subscriptionsGuid);
            ...
            {
                var response = new NoContentResult();
                _logger.LogInformation("{CorrelationId} Response Status Code: {StatusCode}. Subscriptions does not exist for customer {customerGuid} and subscription with {subscriptionsGuid}", correlationId, response.StatusCode, customerGuid, subscriptionsGuid);                
                return response;
            }

            _logger.LogTrace("{CorrelationId} Attempting to Patch Subscription with ID {subscriptionsGuid}", correlationId, subscriptionsGuid);
            ...
            if (updatedSubscriptions == null)
            {
                ...
                _logger.LogInformation("{CorrelationId} Response Status Code: {StatusCode}. Failed to patch the Subscription with ID {subscriptionsGuid}", correlationId, response.StatusCode, subscriptionsGuid);
